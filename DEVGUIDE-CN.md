...
### composer 方式
a.在项目的根目录运行 composer require alibabacloud/oss-v2，或者在composer.json文件中添加如下依赖关系。
```
"require": {
    "alibabacloud/oss-v2": "~0.2.0"
}
```
b.运行composer install，安装依赖。安装完成后，目录结构如下：
```
        .
        ├── src
        |   └──app.php
...
### phar方式
a.在[GitHub](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/releases)中选择相应的版本并下载打包好的phar文件。

b.在代码中引入phar文件：
```
require_once '/path/to/alibabacloud-oss-php-sdk-v2.phar';               
```

### 源码方式
a.在[GitHub](https://github.com/aliyun/alibabacloud-oss-php-sdk-v2/releases)中选择相应的版本并并下载打包好的zip文件。

b.解压后的根目录中包含一个autoload.php文件，在代码中引入此文件：
```
...
```
$cfg = Oss\Config::loadDefault();
$cfg->setRegion('cn-hangzhou')->setInsecureSkipVerify(true);
...
```
public funtion <operationName>(request: OperationNameRequest, args: $args):Models\<OperationName>Result
```
...
预签名接口定义如下：
```
public function presign(request: OperationNameRequest,args: $args): Models\PresignResult
```
...
基础接口里， 出了PutObject, AppendObject, UploadPart 接口外，会自动计算MD5, 并设置Content-MD5, 保证请求的完整性。
