...
try {
    $request = new Models\GetAgenticBucketEncryptionRequest($bucket);
    $result = $client->getAgenticBucketEncryption($request);
    ...
    if (
        $result->serverSideEncryptionRule !== null &&
        $result->serverSideEncryptionRule->applyServerSideEncryptionByDefault !== null
    ) {
        printf(
            'sse algorithm: %s' . PHP_EOL,
            $result->serverSideEncryptionRule->applyServerSideEncryptionByDefault->sseAlgorithm
        );
    }
} catch (\Throwable $e) {
    $se = null;
