...
try {
    $request = new Models\GetAgenticBucketAclRequest($bucket);
    $result = $client->getAgenticBucketAcl($request);
    ...
    if ($result->accessControlPolicy !== null && $result->accessControlPolicy->accessControlList !== null) {
        printf('acl: %s' . PHP_EOL, $result->accessControlPolicy->accessControlList->grant);
    }
} catch (\Throwable $e) {
    $se = null;
