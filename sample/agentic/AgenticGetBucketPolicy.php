...
try {
    $request = new Models\GetAgenticBucketPolicyRequest($bucket);
    $result = $client->getAgenticBucketPolicy($request);

    printf(
        'status code: %d' . PHP_EOL . 'request id: %s' . PHP_EOL . 'policy: %s' . PHP_EOL,
        $result->statusCode,
        $result->requestId,
        $result->policy ?? ''
    );
