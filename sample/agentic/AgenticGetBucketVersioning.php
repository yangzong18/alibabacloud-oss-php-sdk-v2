...
try {
    $request = new Models\GetAgenticBucketVersioningRequest($bucket);
    $result = $client->getAgenticBucketVersioning($request);
    ...
    if ($result->versioningConfiguration !== null) {
        printf('versioning status: %s' . PHP_EOL, $result->versioningConfiguration->status ?? '');
    }
} catch (\Throwable $e) {
    $se = null;
